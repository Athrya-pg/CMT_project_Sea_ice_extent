...
df_nhcut = pd.DataFrame(nhcut, columns=['NH_Extent'])
df_shcut = pd.DataFrame(shcut, columns=['SH_Extent'])
...
#nhcut['std_NH'] = StandardScaler().fit_transform(np.array(nhcut).reshape(-1, 1))
shcut['std_SH'] = StandardScaler().fit_transform(np.array(shcut).reshape(-1, 1))

# Ensuite, effectuer la régression sur x_scaled et y_scaled

print('creating csv file')
nhcut.to_csv('sea_ice_nh.csv', index=False)
shcut.to_csv('sea_ice_sh.csv', index=False)
